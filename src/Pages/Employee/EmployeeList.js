...
import EmpDelete from './EmployeeDelete';
...
const EmployeeList = () => {
    ...
    axios.get(EMPLOYEE_API, { headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem("token"))}` } })
      .then((res) => {
        if (res.status === 200) {
          setData(res.data);
          res.data.map((item) => {
            return (setApidepartment(item.department))
          })
          setLoader(false)
        } else {
          console.log("error")
  ...
  const FilterData = (event) => {
    switch (event) {
      case "All":
        const AllData = departmentapi;
        setSearchItems(AllData)
        return console.log(AllData);
      case "IT":
        const result = departmentapi.filter(word => word.department === "IT");
        setSearchItems(result)
        return console.log(result);
      case "Sales":
        const result2 = departmentapi.filter(word => word.department === "Sales");
        setSearchItems(result2)
        return console.log(result2);
      case "Digital Marketing":
        const result3 = departmentapi.filter(word => word.department === "Digital Marketing");
        setSearchItems(result3)
        return console.log(result3);
      case "Quality Analyst":
        const result4 = departmentapi.filter(word => word.department === "Quality Analyst");
            ...
            <div className='row'>
              <div className='col-lg-3 mb-4'>
                <Form.Select onChange={FilterData}>
                  ...
                  {data.map((item, i) => {
                    return (<option value={item.deprtment} key={i}>{item.department}</option>)
                  })}
                </Form.Select>
              </div>
            </div>
